            ...
            steps {
                ...
                script {
                    if (env.MYBRANCH != 'develop') {
                        ...
                        // previously triggered external 'aethersonarscanner' job here;
                        // Sonar scan is now inlined as its own stage (see 'Sonar Scan' stage below)
                        echo 'Skipping external aethersonarscanner trigger; sonar will run in-pipeline.'
                    }
                }
            }
        }

        // Inline the former aethersonarscanner job as a declarative stage
        stage('Sonar Scan') {
            agent any
            when {
