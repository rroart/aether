...
public class ZKMessageUtil {
	...
	try {
	    List<String> nodes = ZKInitialize.zk.getChildren(ZKUtil.getAppidPath() + Constants.NODES, false);
	    for (String node : nodes) {
		if (!nodename.equals(node)) {
			ZKInitialize.createTempIfNotExists(ZKInitialize.zk, ZKUtil.getAppidPath(Constants.NODES) + node + "/" + Constants.REFRESH);
    ...
    try {
        ...
        for (String node : nodes) {
        if (!nodename.equals(node)) {
            ZKInitialize.createTempIfNotExists(ZKInitialize.zk, ZKUtil.getAppidPath(Constants.NODES) + node + "/" + Constants.RECONFIG);
